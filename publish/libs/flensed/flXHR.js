...
(function (global)
{
    ...
    _flensed.flXHR = function (configObject)
    {
                ...
                if (instancePooling)
                {
                    var ret = function ()
                    {
                        for (var k = 0; k < _flxhr_inp.length; k++)
                        {
                            ...
                            if (inst[_READYSTATE_] === 4)
                            {	// must have already been used and be idle
                                inst.Reset();
        ...
        function continueConstructor()
        {
            ...
            try { appendTo.nodeName.toLowerCase(); _flensed.checkplayer.module_ready(); _chkplyr = _flensed.checkplayer; } catch (err)
            {	// make sure DOM object and checkplayer are ready
        ...
        function stillContinueConstructor()
        {
            ...
            if (_flxhr_css === null && appendToId === null)
            {	// only if CSS hasn't been defined yet, and if flXHR's being added to the BODY of the page
        ...
        {
            if (binaryResponseBody && typeof arguments[0] !== UNDEF)
            {	// most likely an array of byte parameters for binary response
                rawresponse = ((rawresponse !== null) ? rawresponse : []);
                rawresponse = rawresponse.concat(arguments[0]);
            }
            else if (typeof arguments[0] === JSSTR)
        ...
        {
            updateFromPublicAPI();
            if (readyState <= 1 && !_error)
            {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0)
        ...
        function setRequestHeader()
        {
            ...
            {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0), or if the send has already been called
