/*
	Copyright (c) 2010, City Index Ltd - All Rights Reserved.
	Available via [insert license terms]
	see: [insert license url] for details
	(need to fold in the dojo licensing)
*/
...
(function () {
    ...
    dojo.xhr = function (/*String*/method, /*dojo.__XhrArgs*/args, /*Boolean?*/hasBody) {
        ...
        // SKY: we should use CORS differentiator and feature detection to avoid setting headers when they
        // 1) not appropriate and/or 2) break the transport
        ...
        // FIXME: is this appropriate for all content types?
                    ...
                    // SKY: HACK: webkit does not like content-type on CORS GET
            // this really should use a cors differentiation and browser sniffing
            // but for now just enable us to not set content-type from our code by
            // setting a null
        ...
        if (envDef.namedParams) {
            ...
            if ((args.length == 1) && dojo.isObject(args[0])) {
                ...
                for (var i = 0; i < method.parameters.length; i++) {
                    if (typeof args[i] != "undefined" || (smd.SMDVersion > 2 ? method.parameters[i].required : !method.parameters[i].optional)) { // SKY: JSON SCHEMA v3 patch
            ...
            for (i = 0; i < parameters.length; i++) {
                ...
                if ((smd.SMDVersion > 2 ? param.required : !param.optional) && param.name && !args[param.name]) { // SKY: JSON SCHEMA v3 patch
                    if (param["default"]) {
...
dojox.rpc._getContentTypes = function (smd, method, defaultContentType) {
    /// <summary>
    /// 
    /// 
    /// </summary>
    ...
    // TODO: this content type related code should be abstracted and applied to each envelope
    var contentType = smd.contentType || method.contentType || defaultContentType;

    var handleAs;
    var responseContentType = (smd.responseContentType || method.responseContentType || defaultContentType).toLowerCase();
    ...
    if (responseContentType.indexOf("xml") > -1) {
        handleAs = "xml";
    }

    return { contentType: contentType, handleAs: handleAs };
}

dojox.rpc.appendQuery = function (url, query) {
    /// <summary>
    /// 
    /// 
    /// </summary>
...
dojox.rpc.applyUriTemplate = function (target, data) {
    /// <summary>
    /// Applies values from <paramref="data"/> to named 'template' place holders in <paramref="target"/>
    /// returning the modified target and unused data elements.
    /// </summary>
    ...
    for (var key in data) {
        ...
        if (uriTemplateRx.test(target)) {
            var value = data[key] || "";
            target = target.replace(uriTemplateRx, encodeURIComponent(value));
        }
        else {
    ...
    // eliminate orphaned templates. may result in '&&' but this is functionally valid
...
dojox.rpc.envelopeRegistry.register(
	...
	function (str) { return str == "URL"; },
	{
	    serialize: function (smd, method, data) {
	        ...
	        var temp = { target: method.target, data: data };
	        var uriTemplate = smd.uriTemplate || method.uriTemplate;

	        if (uriTemplate) {
	            temp = dojox.rpc.applyUriTemplate(uriTemplate, data);
	        }

	        var q = temp.target;
	        var d = dojo.objectToQuery(temp.data);

	        var contentTypes = dojox.rpc._getContentTypes(smd, method, "");

	        return {
	            data: d,
	            contentType: contentTypes.contentType,
	            handleAs: contentTypes.handleAs,
	            transport: "POST", // ? why is transport being set here? is this default?
	            uriTemplate: q
	        };
	    },
...
dojox.rpc.envelopeRegistry.register(
	...
	function (str) { return str == "JSON"; },
	{
	    serialize: function (smd, method, data) {
	        ...
	        if (smd.uriTemplate || method.uriTemplate) {
	            temp = dojox.rpc.applyUriTemplate((smd.uriTemplate || method.uriTemplate), data);
	        }
	        var q = temp.target;
	        var d = dojo.toJson(temp.data);

	        var contentTypes = dojox.rpc._getContentTypes(smd, method, "application/json");
...
dojox.rpc.transportRegistry.register(
	...
	function (str) { return str == "JSONP"; },
	{
	    fire: function (r) {
	        ...
	        r.url = dojox.rpc.appendQuery(r.target, r.data);
	        r.callbackParamName = r.callbackParamName || r.jsonpCallbackParameter || "callback";
